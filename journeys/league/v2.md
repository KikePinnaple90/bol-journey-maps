...
> A punter tries to find pre-match, live, or outright markets for a specific league (e.g. NFL, Premier League) in a single session
...
#### Pain Points
- Homepage includes promotion; navigation to specific leagues (which is not available unless league is popular in mobile, not in desktop without left nav), access to only to most popular games without league navigation
- No persistent "recently viewed leagues" shortcut
- Live and Pre-match are presented separated, creating confusion about where to start
...
- Top Leagues or Favorite Leagues nav in desktop
...
The user clicks on the sport (e.g. Football / Soccer) from the left nav or in the league from mobile top menu. They expand the sport in left nav that lists available leagues.
...
- No indication if leagues have live games or outrights or if is only pre-match available
- No indications of where all content for the league are available
- Users find the navigation too slow; they prefer direct URLs or search

#### Opportunities
- Reduce left nav items and group all league content related inside one single elft nav entry point 
- Show live/pre-match/outright availability badges next to each league name

#### Ideas
- "Now Live" indicator on league rows
- Create a League Hub with all league related content. Also 2nd half lines when available, SGPs and Odds Boosters
...
#### Pain Points
- No unified league content — clicking a league in Pre-match takes you to pre-match only; clicking in Live takes you to live only
- Outrights are in a completely separate section, often hard to find
- Different Outrights from same league are not even in the same view and user needs to navigate in loops in the left nav 
- Users have to navigate to 2-3 or even more different places to get a full picture of a league's content 

#### Opportunities
- Build a unified League page: tabs for Pre-match / Live / Outrights in one URL
...
- Create a Personalized Hub based on bet history for users "My Hub" where they can find all the leagues they like to bet on
...
- Left nav counter doesn´t represent real games number
- Live carousel with non related leagues and even sport are displayed at the top of the league
- Live games that have started are not available, causing confusion

#### Opportunities
- Merge duplicated games into one single game entry
- Unified left nav entries for same league
- Add a "Same Game Parlay" and "Odds Boosters" filtered content CTA on league view for recreational bettors
- Cross selling: Remove non related league content unless sure is relevant the user 

#### Ideas
- Unified all league related content in one entry point
- Prematch to live transitions
- Offer relevant cross sell opportuinities 
...
**Success Criteria:** User finds their live game within 10 seconds of deciding to bet live  

#### Pain Points
- No direct access from the league page to live markets for that league
- Sports are not ordered by popularity, depending on human action to order them in the backend properly
...
#### Opportunities
- Add live games directly on the league page togethter with all league content related, to access live now content
- Persist league filter when user switches from pre-match to live
- Order content in live betting based in popularity
- Take prematch popularity to live betting

#### Ideas
- Live games are in the league hub
- Prematch to live transition in both league hub view and event view
- Leagues order by popularity in live
- Use Most Popular prematch algorithim to populate popular sports, leagues and games in live as soon as they are available, instead of wating for the to have traction.
...
- Access to outrights is fragmeneted in multiple left nav entry points
- In soccer Outrights are not even inside the sport tree

#### Opportunities
- Add all Outrights related to a league on the league hub with an outright tab
- Split all outrights in sub tabs
